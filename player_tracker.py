...
def match_line(target_line, candidate_lines, tol_angle=30, tol_offset=60):
    ...
    for pts in candidate_lines:
        ...
        d_offset = min(
            point_line_distance(target_eq, (x1, y1)),
...
class player_tracker:
    ...
    def track_players(self, video_path):
        ...
        while True:
            ...
            if on_first_frame:
                ...
                line1_pts, line2_pts = find_two_lines_closest_to_center(
                    lines, image_center
                )
                ...
                init_distance = perpendicular_distance_between_parallel_lines(
                    line1_eq, line2_eq
                )
            ...
            curr_distance = perpendicular_distance_between_parallel_lines(
                curr_line1_eq, curr_line2_eq
            )
            ...
            for i in new_positions:
                scaled_new_positions.append(
                    transform_point(i, scale_factor, (translation_x, 0))
                )
    ...
    def save_player_tracking_with_first_frame(
        self, video_path, first_frame_path, final_map_path
    ):
        static_frame, frame = self.track_players(video_path)
        cv2.imwrite(first_frame_path, static_frame)
        cv2.imwrite(final_map_path, frame)
